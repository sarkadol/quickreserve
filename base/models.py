...
class Unit(models.Model):
    ...
    belongs_to_category = models.ForeignKey(Category, on_delete=models.CASCADE) #belongs to 4people rooms
    class Meta:
        unique_together = ('unit_name', 'belongs_to_category') #two units can have same name if they are in different category

    def __str__(self):
        return self.unit_name #rewrites Category object (x) to its name in admin panel
    
class ReservationSlot(models.Model):
    unit = models.ForeignKey(Unit, related_name='reservation_slots', on_delete=models.CASCADE)
